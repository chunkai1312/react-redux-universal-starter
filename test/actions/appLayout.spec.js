import { expect } from 'chai'
import * as actions from '../../src/actions/appLayout'
import * as types from '../../src/constants/actionTypes'

describe('Layout Actions:', () => {
  describe('#setPageTitle()', () => {
    it('should be a function', () => {
      expect(actions.setPageTitle).to.be.a('function')
    })

    it('should create an action with type "SET_PAGE_TITLE" and payload', () => {
      const title = 'Home'
      expect(actions.setPageTitle(title)).to.eql({ type: types.SET_PAGE_TITLE, payload: title })
    })
  })

  describe('#toggleNavDrawer()', () => {
    it('should be a function', () => {
      expect(actions.toggleNavDrawer).to.be.a('function')
    })

    it('should create an action with type "TOGGLE_NAV_DRAWER"', () => {
      expect(actions.toggleNavDrawer()).to.eql({ type: types.TOGGLE_NAV_DRAWER })
    })
  })

  describe('#toggleBackButton()', () => {
    it('should be a function', () => {
      expect(actions.toggleBackButton).to.be.a('function')
    })

    it('should create an action with type "TOGGLE_BACK_BUTTON"', () => {
      expect(actions.toggleBackButton()).to.eql({ type: types.TOGGLE_BACK_BUTTON })
    })
  })

  describe('#clickMenuItem()', () => {
    it('should be a function', () => {
      expect(actions.clickMenuItem).to.be.a('function')
    })

    it('should create an action with type "CLICK_MENU_ITEM" and payload', () => {
      const menuItem = { title: 'Home', icon: 'home', path: '/' }
      expect(actions.clickMenuItem(menuItem)).to.eql({ type: types.CLICK_MENU_ITEM, payload: menuItem })
    })
  })

  describe('#clickBackButton()', () => {
    it('should be a function', () => {
      expect(actions.clickBackButton).to.be.a('function')
    })

    it('should create an action with type "CLICK_BACK_BUTTON"', () => {
      expect(actions.clickBackButton()).to.eql({ type: types.CLICK_BACK_BUTTON })
    })
  })

  describe('#backToHome()', () => {
    it('should be a function', () => {
      expect(actions.backToHome).to.be.a('function')
    })

    it('should create an action with type "BACK_TO_HOME"', () => {
      expect(actions.backToHome()).to.eql({ type: types.BACK_TO_HOME })
    })
  })
})
